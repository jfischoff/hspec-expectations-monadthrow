...
module Test.Hspec.Expectations.MonadThrow (
...
-- import           Test.HUnit.Lang (HUnitFailure (..), Location (..))
...
import           Data.CallStack (HasCallStack, SrcLoc (..), callStack, CallStack)
#else
...
callStackList :: HasCallStack => [([Char], SrcLoc)]
callStackList = GHC.getCallStack $ GHC.callStack
#else
...
assertFailure :: (HasCallStack, MonadThrow m)
              => String -- ^ A message that is displayed with the assertion failure
              -> m ()
assertFailure msg = msg `deepseq` throwM (HUnitFailure location $ Reason msg)

type Expectation m = m ()

expectationFailure :: (HasCallStack, MonadThrow m) => String -> Expectation m
expectationFailure = assertFailure

expectTrue :: (HasCallStack, MonadThrow m) => String -> Bool -> Expectation m
...
(@?=) :: (HasCallStack, Eq a, Show a, MonadThrow m)
...
-- to @expected@.
shouldBe :: (HasCallStack, Show a, Eq a, MonadThrow m) => a -> a -> Expectation m
actual `shouldBe` expected = actual @?= expected

-- |
-- @v \`shouldSatisfy\` p@ sets the expectation that @p v@ is @True@.
shouldSatisfy :: (HasCallStack, Show a, MonadThrow m) => a -> (a -> Bool) -> Expectation m
v `shouldSatisfy` p = expectTrue ("predicate failed on: " ++ show v) (p v)

compareWith :: (HasCallStack, Show a, MonadThrow m) => (a -> a -> Bool) -> String -> a -> a -> Expectation m
...
shouldStartWith :: (HasCallStack, Show a, Eq a, MonadThrow m) => [a] -> [a] -> Expectation m
...
shouldEndWith :: (HasCallStack, Show a, Eq a, MonadThrow m) => [a] -> [a] -> Expectation m
...
shouldContain :: (HasCallStack, Show a, Eq a, MonadThrow m) => [a] -> [a] -> Expectation m
...
shouldMatchList :: (HasCallStack, Show a, Eq a, MonadThrow m) => [a] -> [a] -> Expectation m
...
-- returns @expected@.
shouldReturn :: (HasCallStack, Show a, Eq a, MonadThrow m) => m a -> a -> Expectation m
...
shouldNotBe :: (HasCallStack, Show a, Eq a, MonadThrow m) => a -> a -> Expectation m
...
shouldNotSatisfy :: (HasCallStack, Show a, MonadThrow m) => a -> (a -> Bool) -> Expectation m
...
shouldNotContain :: (HasCallStack, Show a, Eq a, MonadThrow m) => [a] -> [a] -> Expectation m
...
shouldNotReturn :: (HasCallStack, Show a, Eq a, MonadThrow m) => m a -> a -> Expectation m
...
-- with a 'Selector'.
shouldThrow :: (HasCallStack, Exception e, MonadCatch m) => m a -> Selector e -> Expectation m
